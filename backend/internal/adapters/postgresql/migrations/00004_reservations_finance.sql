...
CREATE TABLE IF NOT EXISTS
    finance.tax_rules (
        ...
        tax_percentage NUMERIC(5, 2) NOT NULL CHECK (
            tax_percentage>=0.00
            AND tax_percentage<=75.00
        ), -- e.g., 10.00 for 10%
...
CREATE TABLE IF NOT EXISTS
    sales_ledgers.accounts (
        ...
        FOREIGN KEY (property_id, company_profile_id) REFERENCES identity.company_profiles (property_id, id) ON DELETE SET NULL
    );
...
CREATE TABLE IF NOT EXISTS
    operations.reservation_groups (
        ...
        code TEXT GENERATED ALWAYS AS ('GRP-'||LPAD(sequential::TEXT, 5, '0')) STORED, -- e.g., GRP-00123
...
CREATE TABLE IF NOT EXISTS
    operations.reservation_items (
        ...
        stay_period TSTZRANGE NOT NULL CHECK (
            lower(stay_period) IS NOT NULL
            AND upper(stay_period) IS NOT NULL
            AND lower(stay_period)>NOW()
        ), -- [check-in, check-out)
        ...
        CONSTRAINT no_overlapping_room_stays EXCLUDE USING GIST (
            ...
            FOREIGN KEY (property_id, booked_room_type_id) REFERENCES inventory.room_types (property_id, id) ON DELETE SET NULL,
            -- Ensures the room belongs to the same property
            FOREIGN KEY (property_id, assigned_room_id) REFERENCES inventory.rooms (property_id, id) ON DELETE SET NULL,
...
CREATE TRIGGER trg_check_room_occupancy BEFORE INSERT
OR
UPDATE ON operations.reservation_items FOR EACH ROW
EXECUTE FUNCTION operations.fn_validate_room_occupancy ();
...
CREATE TABLE IF NOT EXISTS
    relations.reservation_item_guests (
        ...
        FOREIGN KEY (property_id, guest_id) REFERENCES identity.guests (property_id, id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS
    pricing.booked_daily_rates (
        ...
        final_price_pence INT NOT NULL GENERATED ALWAYS AS (
            CASE
                WHEN adjustment IS NULL
                OR adjustment='{}' THEN base_price_pence
...
OR
UPDATE ON pricing.booked_daily_rates FOR EACH ROW
EXECUTE FUNCTION pricing.fn_calculate_final_price ();
...
CREATE TABLE IF NOT EXISTS
    finance.folios (
        ...
        balance_pence INT NOT NULL DEFAULT 0 CHECK (balance_pence>=0), -- Current balance in pence
        created_at TIMESTAMPTZ DEFAULT NOW(),
        updated_at TIMESTAMPTZ DEFAULT NOW(),
        deleted_at TIMESTAMPTZ DEFAULT NULL, -- For soft deletes
        UNIQUE (property_id, id),
        ...
        -- Ensure the sales ledger account belongs to the same property
        FOREIGN KEY (property_id, sales_ledger_id) REFERENCES sales_ledgers.accounts (property_id, id) ON DELETE SET NULL
    );
...
CREATE TABLE IF NOT EXISTS
    finance.folio_transactions (
        ...
        tax_amount_pence INT GENERATED ALWAYS AS (
            CAST(
                (net_unit_price_pence*quantity)*tax_rate_snapshot/100 AS INT
            )
        ) STORED,
        gross_amount_pence INT NOT NULL GENERATED ALWAYS AS (
            net_unit_price_pence*quantity+(
                CAST(
                    (net_unit_price_pence*quantity)*tax_rate_snapshot/100 AS INT
                )
            )
        ...
        posted_by_user_id UUID REFERENCES auth.users (id) ON DELETE SET NULL CHECK (
            posted_at IS NOT NULL
            OR posted_by_user_id IS NULL
        ),
...
CREATE TABLE IF NOT EXISTS
    finance.invoices (
        ...
        property_code TEXT NOT NULL DEFAULT 'RES' CHECK (
            LENGTH(property_code)=3
            OR LENGTH(property_code)=4
        ), -- Snapshot of property code at time of invoice
        ...
        deleted_at TIMESTAMPTZ DEFAULT NULL, -- For soft deletes
        -- Ensure the folio belongs to the same property
        FOREIGN KEY (property_id, folio_id) REFERENCES finance.folios (property_id, id) ON DELETE SET NULL,
        UNIQUE (property_id, id),
...
CREATE INDEX idx_sales_ledger_transactions_type ON sales_ledgers.transactions (
    type
);
...
CREATE INDEX idx_availability_date ON inventory.room_inventory_ledger (calendar_date)
WHERE
    status='available';
...
-- +goose Down
ALTER TABLE operations.reservation_groups
DROP COLUMN IF EXISTS master_folio_id;
...
DROP TABLE IF EXISTS operations.reservations CASCADE;
